...
SecurityMonitor::SecurityMonitor(const RuntimeEnvironment* renv) : RR(renv), _lastCleanup(0)
{
...
void SecurityMonitor::recordSecurityEvent(void* tPtr, const InetAddress& sourceIP, const Address& sourceZT, SecurityEventType eventType, const char* description, const char* packetInfo)
{
	...
	SecurityEvent event(now, sourceIP, sourceZT, eventType, newThreatLevel, description ? description : "");
	if (packetInfo) {
...
std::string SecurityMonitor::exportPrometheusMetrics()
{
	...
	for (const auto& pair : threatCounts) {
		metrics << "zt_security_threat_levels{level=\"" << _threatLevelToString(pair.first) << "\"} " << pair.second << "\n";
	}
	...
	std::sort(sortedIPs.begin(), sortedIPs.end(), [](const auto& a, const auto& b) {
		return (a.second.authFailures + a.second.rateLimitViolations + a.second.invalidPackets + a.second.protocolViolations) > (b.second.authFailures + b.second.rateLimitViolations + b.second.invalidPackets + b.second.protocolViolations);
	});
	...
	for (size_t i = 0; i < maxIPs; ++i) {
		...
		metrics << "zt_security_ip_violations{ip=\"" << ip << "\",type=\"auth_failure\"} " << stats.authFailures << "\n";
		metrics << "zt_security_ip_violations{ip=\"" << ip << "\",type=\"rate_limit\"} " << stats.rateLimitViolations << "\n";
		metrics << "zt_security_ip_violations{ip=\"" << ip << "\",type=\"invalid_packet\"} " << stats.invalidPackets << "\n";
		metrics << "zt_security_ip_violations{ip=\"" << ip << "\",type=\"protocol_violation\"} " << stats.protocolViolations << "\n";
	}
...
SecurityMonitor::ThreatLevel SecurityMonitor::_calculateThreatLevel(IPStats& stats, uint64_t now)
{
	...
	if (hoursActive < 0.01)
		return THREAT_LOW;	 // Less than 36 seconds
	...
	if (authFailureRate >= AUTH_FAILURE_THRESHOLD_HIGH)
		threatScore += 3;
	else if (authFailureRate >= AUTH_FAILURE_THRESHOLD_LOW)
		threatScore += 1;

	if (rateLimitRate >= RATE_LIMIT_THRESHOLD_HIGH)
		threatScore += 3;
	else if (rateLimitRate >= RATE_LIMIT_THRESHOLD_LOW)
		threatScore += 1;

	if (invalidPacketRate >= INVALID_PACKET_THRESHOLD_HIGH)
		threatScore += 3;
	else if (invalidPacketRate >= INVALID_PACKET_THRESHOLD_LOW)
		threatScore += 1;

	if (protocolViolationRate >= 5)
		threatScore += 2;	// Protocol violations are serious
...
void SecurityMonitor::_cleanupOldEntries(uint64_t now)
{
	...
	while (it != _ipStats.end()) {
		if (it->second.lastActivity < cutoffTime && it->second.currentThreatLevel == THREAT_LOW) {
...
const char* SecurityMonitor::_eventTypeToString(SecurityEventType type)
{
	switch (type) {
		...
		case SEC_EXCESSIVE_HELLO:
			return "EXCESSIVE_HELLO";
		case SEC_EXCESSIVE_ECHO:
			return "EXCESSIVE_ECHO";
		case SEC_EXCESSIVE_WHOIS:
			return "EXCESSIVE_WHOIS";
		case SEC_SUSPICIOUS_PATTERN:
