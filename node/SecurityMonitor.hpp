...
class SecurityMonitor {
  public:
	...
	struct IPStats {
		...
		IPStats() : totalPackets(0), authFailures(0), rateLimitViolations(0), invalidPackets(0), protocolViolations(0), lastActivity(0), firstSeen(0), currentThreatLevel(THREAT_LOW)
		{
		}
	};
	...
	struct SecurityEvent {
		...
		SecurityEvent(uint64_t ts, const InetAddress& ip, const Address& zt, SecurityEventType type, ThreatLevel level, const std::string& desc)
			: timestamp(ts)
			, sourceIP(ip)
			, sourceZTAddr(zt)
			, eventType(type)
  ...
  public:
	 ...
	 */
	void recordSecurityEvent(void* tPtr, const InetAddress& sourceIP, const Address& sourceZT, SecurityEventType eventType, const char* description, const char* packetInfo = nullptr);
